...
A monad $M$ on a category $\mathcal{C}$ gives us another category, the **Kleisli category** $\mathcal{C}_\mathbb{M}$. Its arrows are the arrows in $\mathcal{C}$ of the form $f:A\longrightarrow MB$. Because the Kleisli category is ineed a category, we can compose those arrows.
