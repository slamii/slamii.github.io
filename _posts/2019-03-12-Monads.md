...
Consider generating random numbers in a purely functional language like Haskell. Purely functional means that any functin always returns the same result for the same input. Therefore functions like `rand` returning a random number from $(0, 1)$ are not allowed to exist.
...
A function of this form is called a **stateful computation**, `sc`, because what we are doing is passing some state to the function based on which it returns a value together with a new state, in our case some new `rng`. The type of an `sc` is defined to be the type of the return value.
