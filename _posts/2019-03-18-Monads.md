...
$c\ {\scriptstyle >==}\  f$ is an `sc`, name it $c'$. Intuitively, what we want it to do on a state $s$ is to do whatever $c$ did and then do whatever `sc` returned by $f$ does. To be precise the definition of $c'$ is:
